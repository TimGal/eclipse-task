<template>
  ...
  <div v-else>
      ...
      <v-col cols="6">
        ...
        >
          <template v-slot:item="{ props, item }">
            <v-list-item
              v-bind="props"
              :title="`${item.raw.Name} (${item.raw.CharCode})`"
            ></v-list-item>
          </template>
        </v-autocomplete>
      </v-col>
    ...
    <div class="month__name" v-if="getMonthName(13)">
      {{ getMonthName(13) }}
    </div>
    <v-row>
      <v-col cols="6">
      ...
      <v-col cols="6">
        <h2 class="curr__result" v-if="convertedAmount">
          Результат:
          {{
            convertedAmount === "0.00"
              ? "запрашиваемая сумма < 0.01"
              : convertedAmount
          }}
        </h2>
      </v-col>
    </v-row>
    <v-btn class="curr__convert__btn" color="#02111B" @click="calculateCurrency"
      >Конвертировать</v-btn
    >
  </div>
</template>
...
<script setup>
  ...
  const calculateCurrency = () => {
    convertedAmount.value = moneyLib
      .fx(amount.value)
